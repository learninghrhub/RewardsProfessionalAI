...
def read_workbook_bytes(file_bytes: bytes) -> Dict[str, pd.DataFrame]:
    """Read workbook once from bytes. Cached to avoid re-reading on every Streamlit rerun."""
    xls = pd.ExcelFile(io.BytesIO(file_bytes), engine="openpyxl")
    raw_sheets = pd.read_excel(xls, sheet_name=None)
    sheets = {}
    for raw_name, raw_df in raw_sheets.items():
        clean_name = _standardize_sheet_name(raw_name)
        sheets[clean_name] = _clean_dataframe(raw_df)
    return sheets


def read_workbook(file) -> Dict[str, pd.DataFrame]:
    return read_workbook_bytes(file.getvalue())
...
with c1:
    ...
    if uploaded is not None:
        ...
        if uploaded.size > max_size_mb * 1024 * 1024:
        ...
        elif st.session_state.get("uploaded_file_key") != file_key:
            with st.spinner("Reading workbook safely..."):
                try:
            ...
            st.info(f"Workbook already loaded: {uploaded.name}")
with c2:

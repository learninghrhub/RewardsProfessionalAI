...
SOFT_BG = "#F6F8FA"
BORDER = "#D8E3E8"
BRAND_SEQUENCE = [WEF_BLUE, NAVY_BLUE, MEDIUM_BLUE, LIGHT_BLUE, COOL_GRAY, "#4B5563"]

px.defaults.template = "plotly_white"
px.defaults.color_discrete_sequence = BRAND_SEQUENCE
pio.templates.default = "plotly_white"

def enterprise_chart(fig, height=None, y_tickformat=None, x_title=None, y_title=None):
    """Apply a consistent Workday/SAP/Oracle-style chart treatment."""
    fig.update_layout(
        height=height,
        title=dict(font=dict(size=18, color=NAVY_BLUE, family="Arial"), x=0.01),
        font=dict(family="Arial, Helvetica, sans-serif", color=DARK_TEXT, size=13),
        plot_bgcolor="white",
        paper_bgcolor="white",
    ...
    fig.update_xaxes(showgrid=False, zeroline=False, linecolor=BORDER, title=x_title)
    fig.update_yaxes(showgrid=True, gridcolor="#E8EEF2", zeroline=False, linecolor=BORDER, title=y_title)
    if y_tickformat:
...
<style>
    ...
    .main .block-container {padding-top: 1.05rem; padding-bottom: 2.2rem; max-width: 1480px;}
    section[data-testid="stSidebar"] {background: linear-gradient(180deg,#FFFFFF 0%,#F6F8FA 100%); border-right: 1px solid #D8E3E8;}
    section[data-testid="stSidebar"] h1, section[data-testid="stSidebar"] h2, section[data-testid="stSidebar"] h3 {color:#003B5C;}
    h1, h2, h3, h4 {font-family: Arial, Helvetica, sans-serif; color:#222222; letter-spacing:-0.02em;}
    .ra-hero {background: linear-gradient(135deg,#003B5C 0%,#005C8A 52%,#0082C5 100%); padding: 30px 34px; border-radius: 10px; color: white; margin-bottom: 18px; box-shadow: 0 12px 30px rgba(0,59,92,0.18); border:1px solid rgba(255,255,255,0.24);}
    .ra-hero h1 {font-size: 2.15rem; margin-bottom: 0.45rem; line-height:1.15; font-weight:800; color:#FFFFFF;}
    .ra-hero p {font-size: 1.0rem; opacity: 0.96; margin: 0; max-width: 1050px;}
    .suite-badge {display:inline-block; background:rgba(255,255,255,0.16); border:1px solid rgba(255,255,255,0.32); padding:5px 10px; border-radius:999px; font-size:0.78rem; font-weight:700; letter-spacing:.04em; text-transform:uppercase; margin-bottom:10px;}
    .enterprise-strip {display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin: 12px 0 18px 0;}
    .enterprise-pill {border:1px solid #D8E3E8; background:#FFFFFF; border-radius:10px; padding:12px 14px; box-shadow:0 2px 10px rgba(0,59,92,0.05);}
    .enterprise-pill b {color:#003B5C;}
    .insight-box {border-left: 5px solid #0082C5; background: #F3FAFE; padding: 14px 18px; border-radius: 10px; margin: 12px 0; border-top:1px solid #D8E3E8; border-right:1px solid #D8E3E8; border-bottom:1px solid #D8E3E8;}
    .privacy-box {background:#F6F8FA; border:1px solid #D8E3E8; padding:13px 15px; border-radius:10px; font-size:0.9rem; color:#222222;}
    .small-note {font-size:0.85rem; color:#8A9A9F;}
    .risk-high {color:#B42318; font-weight:700;}
    .risk-medium {color:#B54708; font-weight:700;}
    .risk-low {color:#027A48; font-weight:700;}
    div[data-testid="stMetric"] {background:#FFFFFF; border:1px solid #D8E3E8; padding:14px 16px; border-radius:10px; box-shadow:0 2px 12px rgba(0,59,92,0.06);}
    div[data-testid="stMetricLabel"] {color:#5F6B70; font-size:.86rem;}
    div[data-testid="stMetricValue"] {font-size: 1.34rem; color:#003B5C; font-weight:800;}
    div[data-testid="stDataFrame"] {border:1px solid #D8E3E8; border-radius:10px; overflow:hidden;}
    .stTabs [data-baseweb="tab-list"] {gap: 4px; border-bottom:1px solid #D8E3E8;}
    .stTabs [data-baseweb="tab"] {height: 42px; border-radius: 8px 8px 0 0; color:#222222; font-weight:600;}
    .stTabs [aria-selected="true"] {background:#F3FAFE; color:#003B5C; border-bottom:3px solid #0082C5;}
    .stButton>button, .stDownloadButton>button {border-radius:8px; border:1px solid #0082C5; color:#003B5C; font-weight:700;}
    .stButton>button[kind="primary"] {background:#0082C5; color:#FFFFFF; border-color:#0082C5;}
    @media (max-width: 900px) {.enterprise-strip {grid-template-columns:1fr;} .ra-hero h1{font-size:1.55rem;}}
</style>
""",
...
def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Standardize column names and key text fields after Excel upload."""
    if df is None:
        return pd.DataFrame()
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]

    # Remove fully blank rows/columns created by Excel formatting.
    df = df.dropna(axis=0, how="all").dropna(axis=1, how="all")

    text_cols = [
        "Employee_ID", "Employee_Name", "Department", "Business_Unit", "Job_Title",
        "Grade", "Performance_Rating", "Eligibility", "Critical_Role", "Flight_Risk",
        "Country", "Currency", "Compa_Zone", "Current_Grade", "Proposed_Grade"
    ]
...
def _standardize_sheet_name(name: str) -> str:
    """Map common sheet-name variants to the expected RewardAI sheet names."""
    ...
    mapping = {
        ...
        "salary_range": "Salary_Structure",
        "salary_ranges": "Salary_Structure",
        "merit_matrix": "Merit_Matrix",
        "merit": "Merit_Matrix",
        "budget_assumptions": "Budget_Assumptions",
        "budget": "Budget_Assumptions",
        "market_benchmark": "Market_Benchmark",
        "market_data": "Market_Benchmark",
        "market": "Market_Benchmark",
        "promotion_nominations": "Promotion_Nominations",
        "promotions": "Promotion_Nominations",
        "promotion": "Promotion_Nominations",
    }
    return mapping.get(normalized, str(name).strip())


def _worksheet_to_dataframe(ws, max_rows: int = 10000) -> pd.DataFrame:
    """Convert one Excel worksheet into a DataFrame using read-only rows.

    This avoids loading every worksheet through pandas and is safer for Streamlit Cloud.
    The first non-empty row is treated as the header. Fully blank rows are ignored.
    """
    ...
    for row in rows_iter:
        ...
        if any(v is not None and str(v).strip() != "" for v in vals):
            ...
            for idx, value in enumerate(vals):
                name = str(value).strip() if value is not None and str(value).strip() != "" else f"Unnamed_{idx+1}"
                # Make duplicate header names unique so pandas does not break downstream logic.
                if name in seen:
                    seen[name] += 1
    ...
    for row_idx, row in enumerate(rows_iter):
        ...
        if all(v is None or str(v).strip() == "" for v in vals):
            continue
        data.append(vals)
    return _clean_dataframe(pd.DataFrame(data, columns=header))


@st.cache_data(show_spinner=False)
def read_workbook_bytes(file_bytes: bytes, file_name: str = "uploaded_workbook.xlsx", max_rows: int = 10000) -> Dict[str, pd.DataFrame]:
    """Read only RewardAI-relevant sheets from uploaded workbook bytes.

    V3 upload fix:
    - Does not auto-read every Excel sheet through pandas.
    - Uses openpyxl read_only mode.
    - Reads only known RewardAI sheet names and aliases.
    - Cached by file bytes/name/max_rows, so it does not reload on every rerun.
    """
    from openpyxl import load_workbook

    allowed_sheets = {
        "Employee_Data", "Salary_Structure", "Merit_Matrix", "Budget_Assumptions",
        "Market_Benchmark", "Promotion_Nominations"
    }
    sheets = {}
    wb = load_workbook(filename=io.BytesIO(file_bytes), read_only=True, data_only=True)
    try:
        for ws in wb.worksheets:
            clean_name = _standardize_sheet_name(ws.title)
            if clean_name in allowed_sheets and clean_name not in sheets:
                sheets[clean_name] = _worksheet_to_dataframe(ws, max_rows=max_rows)
    finally:
        wb.close()

    if not sheets:
        raise ValueError(
            "No RewardAI sheets found. Expected sheets such as Employee_Data, Salary_Structure, "
            "Merit_Matrix, Budget_Assumptions, Market_Benchmark, or Promotion_Nominations."
        )
    return sheets


def read_workbook(file, max_rows: int = 10000) -> Dict[str, pd.DataFrame]:
    file_bytes = file.getvalue()
    return read_workbook_bytes(file_bytes, getattr(file, "name", "uploaded_workbook.xlsx"), max_rows=max_rows)
...
def normalize_percent(series: pd.Series) -> pd.Series:
    """Accept 0.06, 6, or '6%' and return decimal values."""
...
def _rename_aliases(df: pd.DataFrame, aliases: Dict[str, str]) -> pd.DataFrame:
    ...
    for alias, target in aliases.items():
        if target not in df.columns and alias in normalized_cols:
            rename_map[normalized_cols[alias]] = target
...
def calculate_analysis(sheets: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    emp = _clean_dataframe(sheets["Employee_Data"]).copy()
    ss = _clean_dataframe(sheets["Salary_Structure"]).copy()
    mm = _clean_dataframe(sheets["Merit_Matrix"]).copy()
    budget = _clean_dataframe(sheets.get("Budget_Assumptions", pd.DataFrame())).copy()
    market = _clean_dataframe(sheets.get("Market_Benchmark", pd.DataFrame())).copy()
    promos = _clean_dataframe(sheets.get("Promotion_Nominations", pd.DataFrame())).copy()

    # Flexible column aliases for user-uploaded workbooks.
    emp = _rename_aliases(emp, {
        "employee_id": "Employee_ID", "employee_name": "Employee_Name", "name": "Employee_Name",
        "department": "Department", "job_title": "Job_Title", "title": "Job_Title",
        "grade": "Grade", "current_salary": "Current_Base_Salary", "base_salary": "Current_Base_Salary",
        "current_base_salary": "Current_Base_Salary", "performance": "Performance_Rating",
        "performance_rating": "Performance_Rating", "eligible": "Eligibility", "eligibility": "Eligibility",
        "critical_role": "Critical_Role", "flight_risk": "Flight_Risk", "country": "Country", "currency": "Currency",
    })
    ss = _rename_aliases(ss, {
        "grade": "Grade", "min": "Minimum", "minimum": "Minimum", "salary_min": "Minimum",
        "mid": "Midpoint", "midpoint": "Midpoint", "salary_mid": "Midpoint", "market_midpoint": "Midpoint",
        "max": "Maximum", "maximum": "Maximum", "salary_max": "Maximum",
    })
    mm = _rename_aliases(mm, {
        "performance": "Performance_Rating", "performance_rating": "Performance_Rating",
        "zone": "Compa_Zone", "compa_zone": "Compa_Zone", "merit": "Merit_Percent",
        "merit_percent": "Merit_Percent", "merit_%": "Merit_Percent",
    })

    # Add optional columns if missing to prevent upload failures.
    optional_defaults = {
        "Department": "Unassigned", "Job_Title": "Unmapped", "Eligibility": "Eligible",
        "Critical_Role": "No", "Flight_Risk": "Medium", "Country": "", "Currency": "SAR"
    }
    ...
    df["Range_Penetration"] = np.where((df["Maximum"] - df["Minimum"]) > 0, (df["Current_Base_Salary"] - df["Minimum"]) / (df["Maximum"] - df["Minimum"]), np.nan)
    ...
    market_required = {"Job_Title", "Grade", "Market_P50"}
    if not market.empty and market_required.issubset(set(market.columns)):
        market = _rename_aliases(market, {
            "job_title": "Job_Title", "grade": "Grade", "p25": "Market_P25", "market_p25": "Market_P25",
            "p50": "Market_P50", "market_p50": "Market_P50", "p75": "Market_P75", "market_p75": "Market_P75",
            "p90": "Market_P90", "market_p90": "Market_P90",
        })
        ...
        df["Market_Ratio_P50"] = np.where(df["Market_P50"] > 0, df["Current_Base_Salary"] / df["Market_P50"], np.nan)
        df["Market_Ratio_P75"] = np.where(df["Market_P75"] > 0, df["Current_Base_Salary"] / df["Market_P75"], np.nan)
    ...
    if not promos.empty and {"Employee_ID", "Proposed_Grade"}.issubset(set(promos.columns)):
        p = promos.copy()
        if "Promotion_Percent" not in p.columns:
            p["Promotion_Percent"] = 0.08
        p["Promotion_Percent"] = normalize_percent(p["Promotion_Percent"])
    ...
    # Summaries
    # Department and grade compa-ratio are calculated as aggregate salary / aggregate midpoint.
    # This is stable when a grade has multiple midpoint records across countries/structures and is also equal to
    # average employee compa-ratio when every employee in the grade has the same midpoint.
    dept_summary = df.groupby("Department", dropna=False).agg(
        ...
        Salary_Total=("Current_Base_Salary", "sum"),
        Midpoint_Total=("Midpoint", "sum"),
        Average_Employee_Compa_Ratio=("Compa_Ratio", "mean"),
        Average_Market_Ratio_P50=("Market_Ratio_P50", "mean"),
        Average_Merit_Percent=("Merit_Percent", "mean"),
        Proposed_Merit_Cost=("Merit_Increase_Amount", "sum"),
        Lump_Sum_Cost=("Lump_Sum_Amount", "sum"),
    ).reset_index()
    dept_summary["Compa_Ratio"] = np.where(dept_summary["Midpoint_Total"] > 0, dept_summary["Salary_Total"] / dept_summary["Midpoint_Total"], np.nan)
    ...
    grade_summary["Compa_Ratio"] = np.where(grade_summary["Midpoint_Total"] > 0, grade_summary["Salary_Total"] / grade_summary["Midpoint_Total"], np.nan)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("RewardAI Professional | Enhanced enterprise consulting UI · GitHub/Streamlit-ready")

st.markdown(
    f"""
<div class="ra-hero">
<div class="suite-badge">Professional Edition · Fortune 500-style compensation command center</div>
<h1>RewardAI Professional — Compensation Intelligence Platform</h1>
<p>Enterprise-grade salary review, pay positioning, merit planning, market benchmarking, promotion analysis, scenario modelling, and executive reward governance — styled for CHRO, CFO, NRC, and Rewards Consultant review.</p>
</div>
<div class="enterprise-strip">
  <div class="enterprise-pill"><b>01 · Govern</b><br><span class="small-note">Salary range discipline, exceptions, and approval-ready controls.</span></div>
  <div class="enterprise-pill"><b>02 · Differentiate</b><br><span class="small-note">Performance, compa-ratio, and critical-role based pay actions.</span></div>
  <div class="enterprise-pill"><b>03 · Afford</b><br><span class="small-note">Budget utilization, cost scenarios, and CFO-ready affordability view.</span></div>
  <div class="enterprise-pill"><b>04 · Communicate</b><br><span class="small-note">CHRO, NRC, manager, and employee-facing narrative outputs.</span></div>
</div>
""",
    unsafe_allow_html=True,
)
...
with c1:
    ...
    if uploaded is not None:
        max_size_mb = 15
        st.caption(f"Selected file: {uploaded.name} | Size: {uploaded.size/1024/1024:.2f} MB")
        st.info("Click **Process Uploaded Workbook**. The app will read only RewardAI sheets and will not repeatedly reload the file on every screen refresh.")
        process_upload = st.button("Process Uploaded Workbook", type="secondary", use_container_width=True)
        if uploaded.size > max_size_mb * 1024 * 1024:
            st.error(f"Workbook is too large for this demo build ({uploaded.size/1024/1024:.1f} MB). Please use a file below {max_size_mb} MB or remove unused sheets/formatting.")
        elif process_upload:
            file_bytes = uploaded.getvalue()
            file_hash = hashlib.md5(file_bytes).hexdigest()
            if st.session_state.get("uploaded_file_hash") == file_hash:
                st.info(f"Workbook already processed: {uploaded.name}")
            else:
                with st.spinner("Processing workbook safely. Reading only RewardAI sheets..."):
                    try:
                        st.session_state.sheets = read_workbook_bytes(file_bytes, uploaded.name, max_rows=10000)
                        st.session_state.uploaded_file_hash = file_hash
                        st.session_state.uploaded_file_key = f"{uploaded.name}-{uploaded.size}"
                        st.success("Workbook processed successfully. Continue to Data Validation and Pay Positioning.")
                    except Exception as e:
                        st.error(f"Could not read workbook: {e}")
                        st.warning("Use the Download Sample Workbook format first, then replace the sample data while keeping sheet names and column names unchanged.")
with c2:
...
with main_tabs[0]:
    ...
    a, b = st.columns(2)
    with a:
        fig = px.histogram(analysis, x="Compa_Ratio", nbins=12, title="Compa-Ratio Distribution", color_discrete_sequence=[WEF_BLUE])
        fig = enterprise_chart(fig, height=430, y_title="Employee Count", x_title="Compa-Ratio")
        fig.update_xaxes(tickformat=".0%")
        st.plotly_chart(fig, use_container_width=True)
    with b:
        fig = px.pie(analysis, names="Compa_Zone", title="Employees by Compa-Zone", color_discrete_sequence=BRAND_SEQUENCE, hole=0.42)
        fig.update_traces(textposition="inside", textinfo="percent+label", marker=dict(line=dict(color="#FFFFFF", width=2)))
        fig = enterprise_chart(fig, height=430)
        st.plotly_chart(fig, use_container_width=True)
...
with main_tabs[2]:
    ...
    st.markdown("<div class='insight-box'><b>Pay positioning view:</b> The box plot shows employee-level compa-ratio spread by grade. Each dot is an employee. The bar chart shows aggregate grade compa-ratio using total salary divided by total midpoint.</div>", unsafe_allow_html=True)

    fig = px.box(
        analysis.sort_values("Grade"),
        x="Grade",
        y="Compa_Ratio",
        points="all",
        title="Compa-Ratio by Grade",
        hover_data=["Employee_ID", "Employee_Name", "Department", "Job_Title", "Current_Base_Salary", "Midpoint", "Performance_Rating"],
    )
    fig.update_traces(boxmean=True, marker=dict(color=WEF_BLUE, opacity=0.72, size=7), line=dict(color=MEDIUM_BLUE, width=2), fillcolor="rgba(0,130,197,0.22)")
    fig = enterprise_chart(fig, height=540, y_tickformat=".0%", x_title="Grade", y_title="Compa-Ratio")
    st.plotly_chart(fig, use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        fig = px.bar(grade_summary.sort_values("Grade"), x="Grade", y="Compa_Ratio", title="Aggregate Compa-Ratio by Grade", color_discrete_sequence=[WEF_BLUE])
        fig = enterprise_chart(fig, height=430, y_tickformat=".0%", x_title="Grade", y_title="Aggregate Compa-Ratio")
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        fig = px.bar(dept_summary.sort_values("Compa_Ratio"), x="Department", y="Compa_Ratio", title="Aggregate Compa-Ratio by Department", color_discrete_sequence=[MEDIUM_BLUE])
        fig = enterprise_chart(fig, height=430, y_tickformat=".0%", x_title="Department", y_title="Aggregate Compa-Ratio")
        st.plotly_chart(fig, use_container_width=True)
...
with main_tabs[3]:
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig = px.bar(dept_summary, x="Department", y="Average_Market_Ratio_P50", title="Average Market Ratio P50 by Department", color_discrete_sequence=[WEF_BLUE])
        fig = enterprise_chart(fig, height=430, y_tickformat=".0%", x_title="Department", y_title="Avg Market Ratio P50")
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        fig = px.scatter(analysis, x="Compa_Ratio", y="Market_Ratio_P50", color="Department", hover_name="Employee_Name", title="Internal Pay Position vs Market Ratio", color_discrete_sequence=BRAND_SEQUENCE)
        fig.update_traces(marker=dict(size=10, line=dict(width=1, color="#FFFFFF")))
        fig = enterprise_chart(fig, height=430, y_tickformat=".0%", x_title="Compa-Ratio", y_title="Market Ratio P50")
        fig.update_xaxes(tickformat=".0%")
        st.plotly_chart(fig, use_container_width=True)
...
with main_tabs[7]:
    ...
    fig = px.bar(scenarios, x="Scenario", y="Budget Utilization", color="Talent Risk", title="Scenario Budget Utilization", color_discrete_sequence=BRAND_SEQUENCE)
    fig = enterprise_chart(fig, height=460, y_tickformat=".0%", x_title="Scenario", y_title="Budget Utilization")
    st.plotly_chart(fig, use_container_width=True)
...
- Compa-Ratio by Grade = total current base salary in grade / total assigned midpoint in grade. This is stable for mixed structures and equals the average employee compa-ratio when all employees in a grade share the same midpoint

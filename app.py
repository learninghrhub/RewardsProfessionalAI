...
with main_tabs[2]:
    ...
    fig.update_yaxes(tickformat=".0%", title="Compa_Ratio")
    ...
    with col1:
        fig = px.bar(grade_summary.sort_values("Grade"), x="Grade", y="Compa_Ratio", title="Aggregate Compa-Ratio by Grade")
        fig.update_yaxes(tickformat=".0%")
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        fig = px.bar(dept_summary.sort_values("Compa_Ratio"), x="Department", y="Compa_Ratio", title="Aggregate Compa-Ratio by Department")
        fig.update_yaxes(tickformat=".0%")
        st.plotly_chart(fig, use_container_width=True)
...
with main_tabs[3]:
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig = px.bar(dept_summary, x="Department", y="Average_Market_Ratio_P50", title="Average Market Ratio P50 by Department")
        st.plotly_chart(fig, use_container_width=True)
    with col2:
